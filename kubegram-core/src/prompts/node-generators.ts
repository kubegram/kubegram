 ...
 * Ported from kuberag with full per-type templates
 */

import { GraphNodeType } from '../types/enums.js';
import type { NodeInput, Microservice, Database, Cache, MessageQueue, Proxy, LoadBalancer, Monitoring, Gateway } from '../types/graph.js';
...
`;
  ...
  private extractEnvVars(envVars: { name?: string; value?: string }[]): Record<string, string> {
  ...
  private extractSecrets(secrets: { name?: string }[]): Record<string, string> {
    const result: Record<string, string> = {};
...
export class PromptGeneratorFactory {
  ...
  static getGenerator(nodeType: string | GraphNodeType): PromptGenerator {
    const generator = this.generators.get(nodeType as GraphNodeType);
    return generator || this.genericGenerator;
  }

  static generatePrompt(node: NodeInput): string {
    const generator = this.getGenerator(node.nodeType || 'MICROSERVICE');
...
export function getPromptGenerator(nodeType: string | GraphNodeType): PromptGenerator {
