...
export interface CodegenState extends BaseWorkflowState<
  WorkflowStep,
  WorkflowStatus
> {
  // Input graph
...
export interface PlanState extends BaseWorkflowState<
  PlanWorkflowStep,
  PlanWorkflowStatus
> {
  messages: WorkflowMessage[];
...
export interface ValidationState extends BaseWorkflowState<
...
export function createInitialValidationState(
  ...
  return {
    ...
    modelName:
      options.modelName ??
      DEFAULT_MODEL[options.modelProvider ?? ModelProvider.claude],
...
export function addTargetMessage(
  ...
  return {
    ...
    targetMessages: [
      ...
      { nodeId, nodeType, prompt, priority },
    ],
  };
}

export function addValidationError<
  T extends { validationErrors: ValidationError[] },
>(state: T, error: ValidationError): T {
  return {
    ...state,
