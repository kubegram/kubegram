...
export class CodegenUtils {
    ...
    static buildGraphEdges(graph: Graph, options: EdgeBuildingOptions = {}): Graph {
        const {
    ...
    private static _inferConnections(nodes: GraphNode[], rules: ConnectionRule[]): void {
            ...
            for (const src of sources) {
                for (const tgt of targets) {
                    ...
                    const exists = src.edges?.some(
                        e => e.node.id === tgt.id && e.connectionType === rule.connectionType,
                    );
                    if (!exists) {
                        (src.edges ??= []).push({ node: tgt, connectionType: rule.connectionType });
                        if (rule.bidirectional) {
                            (tgt.edges ??= []).push({ node: src, connectionType: rule.connectionType });
                        }
                    }
                }
            }
            ...
            for (const svc of services) {
                for (const dep of deployments) {
                    if (!svc.edges?.some(e => e.node.id === dep.id)) {
                        (svc.edges ??= []).push({ node: dep, connectionType: ConnectionType.SERVICE_EXPOSES_POD });
                    }
                }
            }
            ...
            for (const dep of deployments) {
                for (const pod of pods) {
                    if (!dep.edges?.some(e => e.node.id === pod.id)) {
                        (dep.edges ??= []).push({ node: pod, connectionType: ConnectionType.MANAGES });
                    }
                }
            }
        }
    ...
    private static _getBaseName(name: string): string {
        const suffixes = ['-service', '-svc', '-deployment', '-deploy', '-pod', '-pods', '-ingress', '-configmap', '-secret'];
    ...
    static getNeededInfrastructure(
        ...
        return (desiredGraph.nodes ?? []).filter(desired => {
            ...
            return (!existing || this._nodeConfigurationChanged(desired, existing)) &&
                this._passesFilters(desired, options);
        });
    }

    static extractNodes(graph: Graph, options: NodeExtractionOptions = {}): GraphNode[] {
        const { includeExternal = true, filterByType, minConnections = 0 } = options;
        let nodes = [...(graph.nodes ?? [])];

        if (filterByType?.length) nodes = nodes.filter(n => filterByType.includes(n.nodeType));
        if (!includeExternal) nodes = nodes.filter(n => n.nodeType !== GraphNodeType.EXTERNAL_DEPENDENCY);
        if (minConnections > 0) nodes = nodes.filter(n => (n.edges?.length ?? 0) >= minConnections);

        return nodes;
    }

    private static _nodeConfigurationChanged(desired: GraphNode, existing: GraphNode): boolean {
        if (desired.name !== existing.name || desired.nodeType !== existing.nodeType) return true;
        return JSON.stringify(desired.spec ?? {}) !== JSON.stringify(existing.spec ?? {});
    }

    private static _passesFilters(node: GraphNode, options: NodeExtractionOptions): boolean {
        const { filterByType, minConnections = 0 } = options;
        if (filterByType?.length && !filterByType.includes(node.nodeType)) return false;
        if (minConnections > 0 && (node.edges?.length ?? 0) < minConnections) return false;
        return true;
    }

    static validateGraph(graph: Graph): { isValid: boolean; errors: string[]; warnings: string[] } {
        const errors: string[] = [];
        ...
        if (!graph.nodes || graph.nodes.length === 0) {
            ...
            if (unnamed.length) warnings.push(`${unnamed.length} nodes without names`);

            for (const node of graph.nodes) {
                for (const edge of node.edges ?? []) {
                    if (!edge.node) errors.push(`Node ${node.id} has edge without target node`);
                    else if (!edge.connectionType) errors.push(`Node ${node.id} has edge without connection type`);
                }
            }
        }
